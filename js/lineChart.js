/*本项目中仅用到大数据量面积图和动态数据图，在此注释掉其他图形操作，否则无法显示*/
/*----------------------折线图-----------------------*/
/*
//折线图堆叠
(function(){
...
(function(){
	...
	var base = new Date();
	var oneDay = 24 * 3600 * 1000;  //24小时    3600秒  1000毫秒     算作1天
	var date = [];

	var data = [25]; //[]里为初始值

for (var i = 1; i < 30; i++) {
    var now = new Date(base -= oneDay);    //天数加1
    date.push([now.getFullYear(), now.getMonth()+1, now.getDate()].join('/')); //格式化并存入数组
    data.push(Math.round((Math.random() - 0.5) * 10 + 25));
}
	date.reverse();    //颠倒数组元素顺序
